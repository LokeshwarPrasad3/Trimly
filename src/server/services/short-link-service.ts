...
export const shortLinkService = {
  async listShortLinks(query: ListShortLinksQuery) {
    ...
    const guestIdentity = await guestIdentityRepository.findByToken(payload.guestToken!);
    if (!guestIdentity) {
  ...
  async createShortLink(input: CreateShortLinkRequest) {
    ...
    if (existingLink) {
      throw new AppError(409, "SLUG_ALREADY_IN_USE", "That short link slug is already in use.");
    }
    ...
    const guestLinkCount = await shortLinkRepository.countByGuestIdentityId(guestIdentity.id);
  ...
  async updateShortLink(id: string, input: UpdateShortLinkRequest) {
    ...
    if (payload.slug) {
      const existingLink = await shortLinkRepository.findBySlug(normalizeSlug(payload.slug));

...
export function LinksPageClient() {
  ...
  const filteredLinks = links.filter((link) => {
    ...
    return [link.title ?? "", link.slug, link.originalUrl].some((value) => value.toLowerCase().includes(query));
  });
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/60 bg-[linear-gradient(135deg,_rgba(14,165,233,0.12),_rgba(255,255,255,0.94)_48%,_rgba(34,211,238,0.14))] p-6 shadow-[0_24px_60px_-45px_rgba(15,23,42,0.35)] lg:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Button render={<Link href="/dashboard/create" />}>Create new link</Button>
        </div>
        <div className="mt-6 grid gap-3 lg:grid-cols-[1fr_auto_auto]">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
            <Input className="border-white/60 bg-white/80 pl-9" placeholder="Search title, slug, or destination" value={search} onChange={(event) => setSearch(event.target.value)} />
          </div>
          <Badge className="justify-center rounded-xl bg-sky-50 px-4 py-2 text-sky-700 hover:bg-sky-50">{links.length} total</Badge>
      ...
      <LinksTable
        ...
        emptyDescription={search ? "Try a different keyword or clear the search to see all links." : "Create your first authenticated short link to populate this table."}
      />
    </div>
  );
}

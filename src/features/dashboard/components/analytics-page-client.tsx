...
export function AnalyticsPageClient() {
  const { data: links = [], isLoading: isLinksLoading } =
    useAuthenticatedLinks();
  const mostActiveLink = [...links].sort(
    (first, second) => second.clickCount - first.clickCount
  )[0];
  const { data: clickEvents = [], isLoading: isEventsLoading } =
    useAuthenticatedClickEvents(mostActiveLink?.id ?? "");
  ...
  const isLoading =
    isLinksLoading || (Boolean(mostActiveLink) && isEventsLoading);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950">
              Live workspace analytics for authenticated users.
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
              This page now reads your real links and click events instead of
              mocked chart data.
            </p>
          </div>

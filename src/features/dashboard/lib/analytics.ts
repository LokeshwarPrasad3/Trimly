...
export function formatClickEventLabel(event: ApiClickEvent) {
  ...
  return (
    [ipLabel, agentLabel, deviceLabel].filter(Boolean).join(" | ") ||
    "Direct visit"
  );
}
  ...
  return [
    {
      ...
      change: `${links.length} links`,
    },
    {
      label: "Active links",
      value: activeLinks.toString(),
      change: `${links.length - activeLinks} inactive`,
    },
    {
      label: "Average clicks",
      value: averageClicks.toLocaleString(),
      change: "per link",
    },
    {
      label: "Latest update",
...
export function buildActivityFromLinks(
  links: ApiShortLink[],
  clickEvents: ApiClickEvent[] = []
) {
